\documentclass[doktyp=barbeit]{TUBAFarbeiten}

\usepackage[ngerman,english]{babel}

\usepackage{selinput}% Auswahl der Dateikodierung (ansi,latin1,utf8,...)
	\SelectInputMappings{adieresis={ä},germandbls={ß},Euro={€}}% Zeichenzuordnung für selinput.sty
\usepackage[T1]{fontenc}% Einstellung Fontencoding

\usepackage{csquotes}% Einstellung zu Anführungszeichen; wird von biblatex.sty gefordert
\usepackage[backend=biber,style=numeric,sorting=none,sortlocale=de_DE_phonebook]{biblatex}% bessere Literaturverarbeitung
\addbibresource{literaturquellen.bib}

\usepackage[hidelinks]{hyperref}
\usepackage[table,figure]{hypcap}

\usepackage{tabularx,diagbox}
\usepackage{float}

\usepackage{amsmath}

\usepackage{listings}
\definecolor{gray}{rgb}{0.95,0.95,0.95}
\definecolor{violet}{rgb}{0.5, 0.0, 0.6}
\lstset{ basicstyle=\ttfamily\small, backgroundcolor=\color{gray} }
\lstset{ numbers=left, numberstyle=\tiny, numbersep=2pt }
\lstset{ breaklines=true, keywordstyle=\color{violet} }

%\usepackage{setspace}% Einstellungen Zeilenabstand
	%\onehalfspacing% Einstellungen Zeilenabstand

\setcounter{secnumdepth}{4}
% \setcounter{tocdepth}{4}

% Worttrennung für nicht bekannte Wörter
\input{hyphenation.tex}



\TUBAFFakultaet{Fakultät für Mathematik und Informatik}
\TUBAFInstitut{Institut für Informatik}
\TUBAFLehrstuhl{Lehrstuhl für Betriebssysteme und Kommunikationstechnologien}

%\TUBAFZweitlogo{\includegraphics{thekla_logo.jpg}}

\TUBAFTitel[Entwicklung einer Sicherheitsinfrastruktur zur Bluetooth-Kommunikation]{Entwicklung einer Sicherheitsinfrastruktur zur Bluetooth-Kommunikation zwischen Smartphone und Mikrocontroller}
\TUBAFBetreuer{\,Prof.\,Dr. Konrad Froitzheim}
\TUBAFKorrektor{M.Sc. Jonas Treumer}%TODO Betreuer Lorenzo Neumann
\TUBAFAutor[M. Käsemodel]{Marian Käsemodel}
\TUBAFStudiengang{Angewandte Informatik}
\TUBAFVertiefung{Technik}
\TUBAFMatrikel{62\,412}
\TUBAFDatum[]{\today}%TODO

\begin{document}

\maketitle

\TUBAFErklaerungsseite


\KOMAoptions{
	listof=totoc	% Abbildungs- und Tabellenverzeichnis im Inhaltsverzeichnis
}

\tableofcontents
\newpage
\listoffigures
\newpage
\listoftables

\newpage
\section{Einleitung}
	\label{sec: einleitung}
	\input{sections/einleitung/einleitung_allg.tex}

	\subsection{Problemstellung}
		\label{sec: problemstellung}
		\input{sections/einleitung/problemstellung.tex}

\newpage
\section{Grundlagen zu Bluetooth Low Energy}
\label{sec: grundlagen le}

	\subsection{Überblick}
		\label{sec: le ueberblick}
		\input{sections/grundlagen_ble/ueberblick.tex}

	\subsection{Topologie}
		\label{sec: le topologie}
		\input{sections/grundlagen_ble/topologie.tex}

	\subsection{Verbindungsaufbau}
		\label{sec: le verbindungsaufbau}
		\input{sections/grundlagen_ble/verbindungsaufbau.tex}

	\subsection{Controller}
		\label{sec: le controller}
		\input{sections/grundlagen_ble/controller/controller_allgemein.tex}

		\newpage
		\subsubsection{Physical Layer}
			\label{sec: le phy}

			\paragraph{Physical Channel} \mbox{} \vspace{0.2cm} \\
				\label{sec: le phy channel}
				\input{sections/grundlagen_ble/controller/physical_layer/physical_channel.tex}

			\paragraph{Physical Link} \mbox{} \vspace{0.2cm} \\
				\label{sec: le phy link}
				\input{sections/grundlagen_ble/controller/physical_layer/physical_link.tex}

		\subsubsection{Link Layer}
			\label{sec: le ll}

			\paragraph{Paketstruktur} \mbox{} \vspace{0.2cm} \\
				\label{sec: le ll paketstruktur}
				\input{sections/grundlagen_ble/controller/link_layer/paketstruktur.tex}

			\paragraph{Logical Transport} \mbox{} \vspace{0.2cm} \\
				\label{sec: le ll transport}
				\input{sections/grundlagen_ble/controller/link_layer/logical_transport.tex}

			\paragraph{Logical Link} \mbox{} \vspace{0.2cm} \\
				\label{sec: le ll link}
				\input{sections/grundlagen_ble/controller/link_layer/logical_link.tex}

	\subsection{Host}
		\label{sec: le host}
		\input{sections/grundlagen_ble/host/host_allgemein.tex}

		\subsubsection{Logical Link Control and Adaption Protocol}
			\label{sec: le l2cap}
			\input{sections/grundlagen_ble/host/l2cap.tex}

		\subsubsection{Generic Attribute Profile}
			\label{sec: le gatt}
			\input{sections/grundlagen_ble/host/gatt.tex}

		\subsubsection{Generic Access Profile}
			\label{sec: le gap}
			\input{sections/grundlagen_ble/host/gap.tex}

		\subsubsection{Security Manager}
			\label{sec: le sm}
			\input{sections/grundlagen_ble/host/smp.tex}

	\subsection{Sicherheit}
		\label{sec: le security}
		\input{sections/grundlagen_ble/sicherheit.tex}

\newpage
\section{Infrastruktur}
	\label{sec: infra allg}
	\input{sections/infrastruktur/infrastruktur_allgemein.tex}

	\subsection{Topologie}
		\label{sec: infra topologie}
		\input{sections/infrastruktur/topologie.tex}

	\subsection{Transport}
		\label{sec: infra transport}
		\input{sections/infrastruktur/transport.tex}

	\subsection{Sicherheit}
		\label{sec: infra sicherheit}
		\input{sections/infrastruktur/sicherheit.tex}

	\subsection{Verbindungsaufbau}
		\label{sec: infra verbindungsaufbau}
		\input{sections/infrastruktur/verbindungsaufbau.tex}

\newpage
\section{Implementierung}
	\label{sec: impl allg}
	\input{sections/implementierung/implementierung_allg.tex}

	\subsection{Topologie}
		\label{sec: impl topologie}
		\input{sections/implementierung/topologie.tex}

	\subsection{Ausleihprozess}
		\label{sec: impl ausleih allg}
		\input{sections/implementierung/ausleihprozess/ausleihprozess_allg.tex}

		\subsubsection{Verbindungsaufbau und Autorisierung}
			\label{sec: impl verbindungsaufbau und autorisierung}
			\input{sections/implementierung/ausleihprozess/verbindungsaufbau_und_autorisierung.tex}

		\subsubsection{Erneuter Verbindungsaufbau}
			\label{sec: impl erneuter verbindungsaufbau}
			\input{sections/implementierung/ausleihprozess/erneuter_verbindungsaufbau.tex}

		\subsubsection{Beenden des Ausleihprozesses}
			\label{sec: impl beenden des ausleihprozesses}
			\input{sections/implementierung/ausleihprozess/beenden_des_ausleihprozesses.tex}

		\subsubsection{Einfluss der Infrastruktur}
			\label{sec: impl einfluss der infrastruktur}
			\input{sections/implementierung/ausleihprozess/einfluss_der_infra.tex}

	\subsection{Prototyp}
		\label{sec: impl prototyp allgemein}
		\input{sections/implementierung/prototyp/prototyp_allg.tex}

		\subsubsection{Hardware und Software}
			\label{sec: impl soft hard}
			\input{sections/implementierung/prototyp/soft_hardware.tex}

		\subsubsection{Simulierung der Zertifizierungsstelle}
			\label{sec: impl prototyp zertifizierungstelle}
			\input{sections/implementierung/prototyp/simulierung_der_zertifizierungsstelle.tex}

		\subsubsection{Ablauf der Anwendung für Client und Server}
			\label{sec: impl prototyp anwendung}
			\input{sections/implementierung/prototyp/ablauf_der_anwendung_fuer_client_und_server.tex}

\newpage
\section{Zusammenfassung und Ausblick}
	\input{sections/zusammenfassung_ausblick/zusammenfassung.tex}

\newpage
\section{Anhang}
	\subsection{Beispiel: Sequence Number / Next Expected Sequence Number}
		\label{sec: anhang nesn sn}
		\input{sections/anhang/nesn_sn_beispiel.tex}

	\newpage
	\subsection{Sequenzdiagramm: Verbindungsaufbau der Infrastruktur}
		\label{sec: anhang infra verb aufbau}
		\input{sections/anhang/infra_verb_aufbau.tex}

	\newpage
	\subsection{Referenz zum Repository}
		\label{sec: anhang repo}
		\input{sections/anhang/repo.tex}

\newpage
\printbibliography[heading=bibintoc]

\end{document}